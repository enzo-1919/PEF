...
// Mostrar/ocultar campo de usuario según el tipo de tabla
...
async function crearNuevaTabla() {
    ...
    // Cargar tablas existentes desde el servidor
...
async function mostrarTablasExistentes() {
    ...
    if (contenedor.children.length === 0) {
        contenedor.innerHTML = '<p>No hay tablas creadas aún</p>';
    }
}

// Crear tarjeta visual para cada tabla
...
async function eliminarTabla(tipo, index) {
    ...
    if (guardado) {
        ...
        // Si estamos editando la tabla eliminada, cerrar el editor
        if (tablaEnEdicion) {
            cerrarEditor();
        }
    }
...
// Agregar una nueva lista a la tabla
...
// Renderizar todas las listas en el editor
...
// Crear tabla HTML para los ejercicios
function crearTablaEjercicios(lista, indexLista) {
    ...
    lista.ejercicios.forEach((ejercicio, indexEjercicio) => {
        ...
        inputNombre.onchange = (e) => {
            ejercicio.nombre = e.target.value;
        };
        tdNombre.appendChild(inputNombre);
        row.appendChild(tdNombre);
        
        // Datos de las 6 semanas
        for (let i = 1; i <= 6; i++) {
            ...
            inputSeries.onchange = (e) => {
                ejercicio[semanaKey].series = e.target.value;
            };
            ...
            inputReps.onchange = (e) => {
                ejercicio[semanaKey].repeticiones = e.target.value;
            };
            ...
            inputPeso.onchange = (e) => {
                ejercicio[semanaKey].peso = e.target.value;
            };
...
// Agregar un nuevo ejercicio a una lista
...
// Descargar PDF de la tabla en edición
